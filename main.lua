...
hitbox_5.MouseButton1Click:Connect(function ()
	if loopTp or d1 or option==1 then return end
	d1=true
	task.spawn(function ()
		task.wait(0.29)
		d1=false
	...
	local elapsed=0
	local con
	local start=UDim2.fromScale(0.5,0)
	local ends=UDim2.fromScale(0,0)
	con=RunService.RenderStepped:Connect(function(dt)
		elapsed=elapsed+dt
		local t=math.clamp(elapsed/0.2,0,1)
		local alpha=1-(1-t)^2
		Selector.Position=start:Lerp(ends,alpha)
...
hitbox_4.MouseButton1Click:Connect(function ()
	if loopTp or d1 or option==2 then return end
	d1=true
	task.spawn(function ()
		task.wait(0.29)
		d1=false
	...
	local elapsed=0
	local con
	local start=UDim2.fromScale(0,0)
	local ends=UDim2.fromScale(0.5,0)
	con=RunService.RenderStepped:Connect(function(dt)
		elapsed=elapsed+dt
		local t=math.clamp(elapsed/0.2,0,1)
		local alpha=1-(1-t)^2
		...
		if t >= 1 then
			Selector.Position=UDim2.new(0.5,0,0,0)
			con:Disconnect()
		end
	end)
end)
